...
import uvicorn
import os
from bhv.db import engine, Base, SessionLocal
...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key=os.environ.get('BHV_SECRET','change-me'))
...
@app.post('/api/signup')
async def api_signup(data: Request):
...
@app.post('/upload')
def upload_post(request: Request, file: UploadFile = File(...), narrative: str = Form(''), user=Depends(require_user)):
    db = SessionLocal()
...
@app.post('/api/upload')
async def api_upload(file: UploadFile = File(...), narrative: str = Form(''), request: Request = None):
