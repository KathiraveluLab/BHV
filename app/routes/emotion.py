...
# Module-level constant — defined once for performance
_COLORS = {
    "red":    (255, 0, 0),
    "blue":   (0, 0, 255),
    "green":  (0, 128, 0),
    ...
    for color_info in dominant_colors:
        ...
        if color_name in COLOR_EMOTION_MAP:
            for emotion, score in COLOR_EMOTION_MAP[color_name].items():
                emotion_scores[emotion] = emotion_scores.get(emotion, 0) + score * weight
    if emotion_scores:
        max_score = max(emotion_scores.values())
        if max_score > 0:
            emotion_scores = {k: round(v / max_score, 3) for k, v in emotion_scores.items()}
